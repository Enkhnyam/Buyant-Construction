...
export default function AdminProjectsPage() {
  ...
  const [imageCaptions, setImageCaptions] = useState<{ [key: string]: { mn: string, en: string } }>({})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      uploadedImages.forEach((file, index) => {
        ...
        formDataToSend.append(`imageCaptions[${index}][mn]`, imageCaptions[file.name]?.mn || '')
        formDataToSend.append(`imageCaptions[${index}][en]`, imageCaptions[file.name]?.en || '')
        formDataToSend.append(`imageOrder[${index}]`, String(index))
      })
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                ...
                <label htmlFor="image-upload" className="cursor-pointer">
                  ...
                  <p className="mt-2 text-sm text-gray-600">
                    {getText('Зураг оруулахын тулд дарна уу', 'Click to upload images')}
                  </p>
                </label>
              </div>
                    ...
                    <div key={file.name} className="relative">
                      ...
                      <div className="mt-2 space-y-2">
                        <input
                          ...
                          value={imageCaptions[file.name]?.mn || ''}
                          onChange={(e) => setImageCaptions(prev => ({
                            ...prev,
                            [file.name]: { ...prev[file.name], mn: e.target.value }
                          }))}
                        ...
                        <input
                          ...
                          value={imageCaptions[file.name]?.en || ''}
                          onChange={(e) => setImageCaptions(prev => ({
                            ...prev,
                            [file.name]: { ...prev[file.name], en: e.target.value }
                          }))}
                          className="w-full px-2 py-1 text-xs border border-gray-300 rounded"
                        />
                      </div>
